# engineering-data — machine-usable claims, not a second First Principles kernel
...
**engineering-data** is that claims store. It is private R&D / pilot diligence today. It is deliberately **not** a second First Principles kernel.

Contact: **vardhan@simtooreal.com** · [simtooreal.com](https://simtooreal.com) · [robosynx.com](https://robosynx.com)

## What it stores

engineering-data holds machine-usable claims about mechanical and industrial engineering:
...
1. **Not a second First Principles kernel.** First Principles owns mechanism admission, conservation structure, and the honesty stamp for admitted knowing. engineering-data does **not** write `AdmissionStatus`.
...
Cross-repo verification with First Principles exists so the claims store and the admission kernel stay honest about their division of labor.
...
## Why separate it from First Principles

First Principles answers: *is this a unit-checked mechanism that may be admitted?*

engineering-data answers: *what mechanical/industrial claims do we have, with what rights and provenance, expressed in SI?*

Mixing those jobs into one process creates a temptation: let a data ingest silently become an admission. We refuse that. engineering-data never writes `AdmissionStatus`. First Principles remains the admission authority for knowing; `machine::verify` remains the simulation-admission authority on the FP side.
...
- engineering-data is the **claims and provenance layer**
- First Principles is the **mechanism admission layer**
...
engineering-data is deeper stack: the industrial-claim substrate behind diligence conversations. Public writing about it is encouraged when it stays diligence-honest — provenance, SI, synthetic fixtures named as synthetic, no fake certifications.
